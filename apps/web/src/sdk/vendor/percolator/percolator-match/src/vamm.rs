...
pub enum MatcherKind {
    ...
    Vamm = 1,
    /// PropAMM: vAMM with on-chain guardrails (skew, divergence, utilization)
    PropAmm = 2,
}
...
/// 0       8     magic ("PERCMATC")
/// 8       4     version
/// 12      1     kind (0=Passive, 1=vAMM, 2=PropAMM)
/// 13      3     _pad0
...
impl MatcherCtx {
    ...
    pub fn validate(&self) -> Result<(), ProgramError> {
        ...
        // For vAMM and PropAMM, liquidity_notional_e6 MUST be non-zero
        if (kind == MatcherKind::Vamm || kind == MatcherKind::PropAmm) && self.liquidity_notional_e6 == 0 {
...
// 0       tag                     u8      1     Always 2
// 1       kind                    u8      1     0=Passive, 1=vAMM, 2=PropAMM
...
fn compute_execution(
    ...
    match kind {
        ...
        MatcherKind::PropAmm => compute_propamm_execution(ctx, call),
    }
}
...
/// Compute PropAMM execution (vAMM + on-chain guardrails)
/// 
/// PropAMM adds thin-market protections:
/// - Inventory-aware skew adjustment (widen spreads when inventory imbalanced)
/// - Oracle divergence guard (widen or halt when mark deviates from oracle)
/// - Utilization-based widening (requires OI data from caller, v1 minimal)
fn compute_propamm_execution(
    ctx: &MatcherCtx,
    call: &MatcherCall,
) -> Result<(u64, i128, u32), ProgramError> {
    let req_abs = call.req_size.unsigned_abs();
    ...
    // Start with vAMM base pricing
    let base = ctx.base_spread_bps as u128;
    let fee = ctx.trading_fee_bps as u128;
    
    // Calculate impact (same as vAMM)
    let impact_bps = if ctx.liquidity_notional_e6 == 0 {
        0u128
    } else {
        let fill_notional_e6 = (fill_abs as u128)
            .checked_mul(call.oracle_price_e6 as u128)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        let utilization = fill_notional_e6
            .checked_mul(10_000)
            .and_then(|x| x.checked_div(ctx.liquidity_notional_e6))
            .unwrap_or(10_000);
        let impact_k = ctx.impact_k_bps as u128;
        utilization.checked_mul(impact_k).and_then(|x| x.checked_div(10_000)).unwrap_or(0)
    };

    // PropAMM guardrails: skew adjustment
    let skew_adj_bps = calculate_skew_adjustment(ctx, is_buy);
    
    // PropAMM guardrails: oracle divergence check
    let (divergence_adj_bps, should_halt) = check_oracle_divergence(ctx, call.oracle_price_e6);
    
    // If divergence exceeds threshold, halt (reject trade)
    if should_halt {
        return Ok((call.oracle_price_e6, 0, FLAG_VALID | FLAG_REJECTED));
    }

    // Total spread = base + fee + impact + skew_adj + divergence_adj
    let total_bps = base
        .checked_add(fee)
        .and_then(|x| x.checked_add(impact_bps))
        .and_then(|x| x.checked_add(skew_adj_bps))
        .and_then(|x| x.checked_add(divergence_adj_bps))
        .ok_or(ProgramError::ArithmeticOverflow)?;
    
    let max_total = ctx.max_total_bps as u128;
    let total_bps = core::cmp::min(max_total, total_bps);
...
/// When LP is long (inventory > 0), widen ask (make buying more expensive)
/// When LP is short (inventory < 0), widen bid (make selling more expensive)
fn calculate_skew_adjustment(ctx: &MatcherCtx, is_buy: bool) -> u128 {
    if ctx.max_inventory_abs == 0 {
        return 0;
    }
    
    // Calculate normalized skew (-1 to +1)
    // Positive = LP is long, negative = LP is short
    let inventory = ctx.inventory_base;
    let max_inv = ctx.max_inventory_abs as i128;
    
    // Clamp to avoid overflow
    let skew_num = if inventory.abs() > max_inv {
        if inventory > 0 { max_inv } else { -max_inv }
    } else {
        inventory
    };
    
    // Skew adjustment: widen when inventory is imbalanced
    // Max adjustment: 100 bps (1%) when fully skewed
    const MAX_SKEW_ADJ_BPS: u128 = 100;
    
    if is_buy {
        // Buying increases LP short exposure (decreases inventory)
        // If LP is already long (inventory > 0), widen ask
        if skew_num > 0 {
            let skew_pct = (skew_num as u128).checked_mul(10_000).and_then(|x| x.checked_div(max_inv as u128)).unwrap_or(0);
            skew_pct.checked_mul(MAX_SKEW_ADJ_BPS).and_then(|x| x.checked_div(10_000)).unwrap_or(0)
        } else {
            0
        }
    } else {
        // Selling increases LP long exposure (increases inventory)
        // If LP is already short (inventory < 0), widen bid
        if skew_num < 0 {
            let skew_pct = (-skew_num as u128).checked_mul(10_000).and_then(|x| x.checked_div(max_inv as u128)).unwrap_or(0);
            skew_pct.checked_mul(MAX_SKEW_ADJ_BPS).and_then(|x| x.checked_div(10_000)).unwrap_or(0)
        } else {
...
/// Check oracle divergence and return adjustment + halt flag
/// Returns (adjustment_bps, should_halt)
fn check_oracle_divergence(ctx: &MatcherCtx, oracle_price_e6: u64) -> (u128, bool) {
    // If no previous execution price, no divergence
    if ctx.last_exec_price_e6 == 0 {
        return (0, false);
    }
    
    let mark = ctx.last_exec_price_e6 as u128;
    let oracle = oracle_price_e6 as u128;
    
    // Calculate divergence in bps
    let diff = if mark > oracle {
        mark - oracle
    } else {
        oracle - mark
    };
    
    // Divergence threshold: 50 bps (0.5%)
    const DIVERGENCE_THRESHOLD_BPS: u128 = 50;
    const HALT_THRESHOLD_BPS: u128 = 100; // 2x threshold = halt
    
    let divergence_bps = diff
        .checked_mul(10_000)
        .and_then(|x| x.checked_div(oracle))
        .unwrap_or(0);
    
    // Halt if divergence exceeds 2x threshold
    if divergence_bps > HALT_THRESHOLD_BPS {
        return (0, true);
    }
    
    // Widen spread if divergence exceeds threshold
    if divergence_bps > DIVERGENCE_THRESHOLD_BPS {
        // Linear adjustment from threshold to halt threshold
        // Max adjustment: 200 bps (2%)
        const MAX_DIVERGENCE_ADJ_BPS: u128 = 200;
        let excess = divergence_bps - DIVERGENCE_THRESHOLD_BPS;
        let excess_pct = excess.checked_mul(10_000).and_then(|x| x.checked_div(DIVERGENCE_THRESHOLD_BPS)).unwrap_or(10_000);
        let adj = excess_pct.checked_mul(MAX_DIVERGENCE_ADJ_BPS).and_then(|x| x.checked_div(10_000)).unwrap_or(MAX_DIVERGENCE_ADJ_BPS);
        return (adj, false);
    }
